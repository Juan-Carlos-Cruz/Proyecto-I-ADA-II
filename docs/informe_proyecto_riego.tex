...
\texttt{iota}=O(n).
\]
Adem\'as, cada invocaci\'on del comparador realiza un n\'umero constante de operaciones aritm\'eticas y comparaciones enteras; por tanto, cada comparaci\'on cuesta \(O(1)\). Como la biblioteca est\'andar garantiza que \texttt{std::sort} realiza \(O(n\log n)\) comparaciones, se obtiene la cota superior
\[
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\textwidth}{l>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X}
    \toprule
    Criterio principal + desempate & \%diff promedio & Diff promedio & Coincidencias exactas \\
    \midrule
