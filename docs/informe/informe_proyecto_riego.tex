...
En espacio, las estructuras dominantes son los arreglos \texttt{dp} y \texttt{eleccion}, ambos indexados por m\'ascara y de tama\~no \(2^n\). Adem\'as, la recursi\'on y el vector del orden reconstruido aportan a lo sumo \(O(n)\) posiciones adicionales. Por ello,
\[
O(2^n) + O(n)=O(2^n),
\]
...
\paragraph{Interpretaci\'on pr\'actica.}
Aunque esta estrategia mejora dr\'asticamente a la fuerza bruta, sigue siendo exponencial. En consecuencia, es apropiada cuando el tama\~no del problema permanece en un rango moderado y se necesita garantizar optimalidad, pero deja de ser viable cuando \(n\) crece lo suficiente como para que \(2^n\) domine tanto el tiempo como la memoria disponibles.
