...
El número de permutaciones posibles de \(n\) tablones es \(n!\). Evaluar una permutación requiere recorrer los \(n\) tablones una vez para calcular el costo, lo que cuesta \(O(n)\). Por lo tanto, el tiempo total del algoritmo es:
\[
O(n! \cdot n).
\]
El espacio auxiliar es \(O(n)\), ya que solo se almacenan la permutación actual, la mejor permutación encontrada y algunas variables escalares.

Esta complejidad factorial hace que el algoritmo sea inviable para valores de \(n\) mayores a \(10\) o \(12\) en la práctica.
