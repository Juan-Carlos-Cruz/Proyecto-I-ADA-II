...
\begin{titlepage}
  ...
  \begin{minipage}{0.82\textwidth}
    ...
    \begin{tabular}{>{\bfseries}p{4cm}p{8.2cm}}
      Estudiantes: & Juan Carlos Cruz Mu\~noz (1824389), David Alejandro Enciso Gutierrez (), Juan Esteban Rodriguez Valencia (), Estiven Andres Martinez Granados (2179687) \\
...
\subsection{Usando fuerza bruta}

El algoritmo de fuerza bruta aborda el problema del riego óptimo de manera exhaustiva: genera **todas** las posibles órdenes de riego (permutaciones de los \(n\) tablones), calcula el costo total de cada una y selecciona la de menor costo. Dado que cualquier solución factible es una permutación, el espacio de búsqueda tiene exactamente \(n!\) elementos.

\subsubsection{Complejidad temporal}

La complejidad temporal del algoritmo está determinada por dos factores:

\begin{enumerate}
  \item \textbf{Número de permutaciones}: hay \(n!\) formas distintas de ordenar los \(n\) tablones.
  \item \textbf{Costo por permutación}: para cada orden se debe simular el riego recorriendo los \(n\) tablones uno tras otro, calculando el costo de cada tablón en tiempo constante (\(O(1)\) por tablón). Por tanto, evaluar una permutación cuesta \(O(n)\).
\end{enumerate}

Multiplicando ambos factores se obtiene la complejidad total:
\[
O(n! \cdot n).
\]

Esta expresión refleja un crecimiento **más que exponencial**: mientras que \(n!\) ya crece extremadamente rápido, el factor adicional \(n\) lo hace aún más pronunciado. Por ejemplo:
\begin{itemize}
  \item Para \(n = 10\): \(10! = 3.628.800\) permutaciones; el costo total es del orden de \(36\) millones de operaciones.
  \item Para \(n = 12\): \(12! \approx 479\) millones de permutaciones; el costo asciende a unos \(5.700\) millones de operaciones.
  \item Para \(n = 15\): \(15! \approx 1.307.674.368.000\) (más de un billón); el costo supera los \(1.96 \times 10^{13}\) operaciones.
\end{itemize}

En la práctica, con un computador moderno que puede ejecutar del orden de \(10^{8}\) operaciones por segundo, una entrada de \(n=12\) ya tomaría varios minutos, mientras que \(n=15\) requeriría años de cómputo. Por esta razón, el algoritmo de fuerza bruta solo es utilizable para valores muy pequeños de \(n\), típicamente \(n \leq 10\) (e incluso para \(n=10\) el tiempo puede ser elevado). Para \(n > 10\) el algoritmo se vuelve completamente impracticable.

\subsubsection{Complejidad espacial}

El espacio auxiliar requerido es lineal en \(n\), es decir, \(O(n)\). Durante la ejecución se almacenan:
\begin{itemize}
  \item la permutación actual (un arreglo de \(n\) enteros),
  \item la mejor permutación encontrada hasta el momento (otro arreglo de \(n\) enteros),
  \item unas pocas variables escalares (costo acumulado, tiempo actual, etc.).
\end{itemize}
No se necesita memoria adicional significativa, por lo que el consumo de memoria no es un factor limitante.

\subsubsection{Corrección}

El algoritmo es totalmente correcto: explora todas las permutaciones, es decir, todas las soluciones factibles del problema. Al evaluar cada una y quedarse con la de menor costo total, garantiza que el resultado devuelto sea la solución óptima global. No existe posibilidad de omitir el mejor orden porque el recorrido es exhaustivo.

\subsubsection{Limitaciones y uso práctico}

Debido a su complejidad factorial, la fuerza bruta se utiliza exclusivamente como **método de referencia** para validar otros algoritmos (voraz y programación dinámica) en instancias muy pequeñas (\(n \leq 8\) en este proyecto). En el contexto de la finca, donde el número de tablones puede ser grande (decenas o cientos), este enfoque resulta inviable. Por tanto, su papel en el proyecto es meramente conceptual y de verificación, no operativo para casos reales de tamaño moderado o grande.

\subsection{Usando un algoritmo voraz}

\subsubsection{Descripci\'on de la Estrategia}
...
El algoritmo voraz ejecuta dos fases: primero ordena los \(n\) tablones según el criterio \(\frac{tr_i}{p_i}\), lo que cuesta \(O(n \log n)\) usando un algoritmo de ordenamiento eficiente; luego evalúa la permutación resultante recorriendo los \(n\) tablones una vez, lo que cuesta \(O(n)\). Por lo tanto, la complejidad temporal total es:\[O(n \log n).\]
El espacio auxiliar es \(O(n)\), debido al arreglo de índices que almacena la permutación resultante.

\subsubsection{Correcci\'on}

El algoritmo voraz \textbf{no siempre} da la respuesta correcta al problema. Aunque el problema tiene subestructura óptima, el criterio local utilizado (menor \(\frac{tr}{p}\)) no satisface la propiedad de escogencia voraz. Existen contraejemplos que demuestran que la decisión localmente óptima puede conducir a una solución global subóptima.
...
\begin{lstlisting}[mathescape=true,basicstyle=\ttfamily\scriptsize]
...
Para evaluar el desempeño de los tres enfoques (fuerza bruta, voraz y programación dinámica) se diseñó una batería de casos de prueba que cubre distintos tamaños de finca y diferentes perfiles de tablones. A continuación se describe el diseño de estos casos y se presentan los resultados obtenidos.

\subsection{Diseño de casos de prueba}

Se construyeron dos tipos de casos de prueba:

\begin{enumerate}
  \item \textbf{Casos reales del repositorio (tests 1 a 18):}  
  Se tomaron las 18 fincas disponibles en el repositorio (\texttt{test1\_in.txt} a \texttt{test18\_in.txt}), cuyas salidas óptimas (\texttt{testX\_out.txt}) fueron proporcionadas por el profesor. Estas fincas tienen tamaños \(n = 5, 6, 7, 8, 10, 15, 20\), lo que permite observar el comportamiento de los algoritmos en escalas pequeñas, medianas y moderadamente grandes.

  \item \textbf{Casos adicionales diseñados manualmente (F1 a F6):}  
  Con el fin de explorar situaciones donde el algoritmo voraz pudiera acertar o fallar, se diseñaron seis fincas adicionales con \(n=5\). En su construcción se variaron deliberadamente los márgenes \(ts-tr\) (holgura), las prioridades y los días perfectos para cubrir perfiles:
  \begin{itemize}
    \item \textbf{Crítico}: holgura cero, solo día perfecto en 0.
    \item \textbf{Urgente}: holgura pequeña (1 a 3) y prioridad alta.
    \item \textbf{Balanceado}: holgura media (2 a 6) y prioridades variadas.
    \item \textbf{Relajado}: holgura grande (7 a 13) y prioridad baja.
    \item \textbf{Largo plazo}: holgura muy grande (10 a 50) y prioridad media.
  \end{itemize}
  Estos casos se incluyen en el análisis para complementar los resultados de la batería principal.
\end{enumerate}
...
\begin{itemize}
  \item \textbf{Fuerza bruta (FB)}: solo para \(n \leq 8\) (tiempo razonable). Devuelve el costo óptimo.
  \item \textbf{Voraz (V)}: siempre ejecutable; se reporta su costo.
  \item \textbf{Programación dinámica (PD)}: ejecutable hasta \(n=20\); devuelve el costo óptimo (idéntico al de FB cuando este es viable).
\end{itemize}

La tabla siguiente resume los resultados para los 18 tests del repositorio. En ella se incluye el costo óptimo (obtenido por FB para \(n\leq8\) y por PD para \(n>8\)), el costo del voraz, la diferencia absoluta y el exceso relativo.
...
\begin{table}[H]
  ...
  \begin{tabular}{cccccccc}
    \toprule
    Test & \(n\) & \multicolumn{1}{c}{Costo FB} & Costo voraz & Costo PD & Diferencia (V-Ópt) & \% exceso & ¿V = Ópt? \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Comparación de costos entre los tres algoritmos. FB solo se ejecutó para \(n \leq 8\) (-- indica no ejecutable en la práctica). El costo PD es el óptimo. La diferencia se calcula respecto al óptimo (PD).}
\end{table}
...
\begin{itemize}
  \item \textbf{Fuerza bruta} solo fue ejecutable para \(n \leq 8\) (tests 1 a 10). En todos esos casos verificó el óptimo.
  \item \textbf{Programación dinámica} produjo el costo óptimo en todos los tests (1 a 18), confirmando su corrección.
  \item \textbf{Voraz} solo alcanzó el óptimo en 1 de los 18 tests (test5, con \(n=6\)). El exceso relativo promedio fue del \(16.23\%\), aunque en los tests de mayor tamaño (\(n=15,20\)) el exceso se redujo (\(<5\%\) en la mayoría). El peor caso relativo se dio en test4 (100\% de exceso), donde duplicó el costo óptimo.
\end{itemize}

\subsection{Ventajas y desventajas de cada enfoque en la práctica}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
    ...
    \textbf{Fuerza bruta} & 
      \begin{itemize}
        \item Garantiza la solución óptima.
        \item Conceptualmente simple.
        \item Útil como validador para tamaños pequeños.
      \end{itemize}
      &
      \begin{itemize}
        \item Complejidad \(O(n! \cdot n)\): inviable para \(n>10\).
        \item No escalable en absoluto.
      \end{itemize}
      \\
    \midrule
    \textbf{Voraz} & 
      \begin{itemize}
        \item Muy rápido: \(O(n \log n)\).
        \item Funciona en cualquier tamaño de entrada.
        \item En la práctica produce costos cercanos al óptimo (exceso relativo bajo en instancias grandes).
      \end{itemize}
      &
      \begin{itemize}
        \item No garantiza optimalidad (puede fallar, especialmente en perfiles relajados o balanceados).
        \item Su precisión depende de la estructura de los datos.
      \end{itemize}
      \\
    \midrule
    \textbf{Programación dinámica} & 
      \begin{itemize}
        \item Garantiza la solución óptima.
        \item Escala hasta \(n\approx 20\) (tiempo y memoria aceptables).
        \item Mucho mejor que fuerza bruta para tamaños moderados.
      \end{itemize}
      &
      \begin{itemize}
        \item Complejidad \(O(n2^n)\): inviable para \(n>25\) en equipos estándar.
        \item Consumo de memoria \(O(2^n)\): para \(n=20\) son ~4 MB, pero para \(n=30\) serían 4 GB.
      \end{itemize}
      \\
    \bottomrule
  \end{tabularx}
  \caption{Ventajas y desventajas de las tres estrategias.}
\end{table}


\section{Conclusiones}
En un entorno real, donde el número de tablones puede ser grande (decenas o cientos), el algoritmo voraz es la única opción viable por su tiempo lineal-logarítmico. Si se requiere garantía de optimalidad y el número de tablones no supera 20, la programación dinámica es la mejor elección. La fuerza bruta queda relegada a un papel de referencia para validar otros algoritmos en instancias muy pequeñas.
