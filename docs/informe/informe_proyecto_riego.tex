...
\begin{lstlisting}[style=pseudobloque]
...
\subsection{Aplicaci\'on del algoritmo}

\subsubsection{Aplicaci\'on a los ejemplos del enunciado}

Tomemos las dos fincas peque\~nas del enunciado:
\[
...
Como ambas instancias tienen \(n=5\), la fuerza bruta eval\'ua exactamente
\[
5! = 120
\]
permutaciones en cada caso. Al recorrer exhaustivamente ese espacio, la funci\'on \texttt{roFB} encuentra las siguientes soluciones \'optimas:
...
\begin{table}[H]
  ...
  \begin{tabular}{cccc}
    \toprule
    Finca & Permutaciones evaluadas & Permutaci\'on \'optima & Costo \'optimo \\
    \midrule
    \(F_1\) & 120 & \(\langle 2,1,0,3,4\rangle\) & 20 \\
    \(F_2\) & 120 & \(\langle 2,1,0,3,4\rangle\) & 32 \\
    \bottomrule
  \end{tabular}
  \caption{Resultado de la b\'usqueda exhaustiva sobre los ejemplos peque\~nos del enunciado.}
\end{table}

En \(F_1\), la mejor permutaci\'on encontrada tiene costo 20; en \(F_2\), el mejor costo es 32. En ambos casos, la salida coincide con la referencia exacta que luego se usa para evaluar al voraz y para contrastar la programaci\'on din\'amica. Esto ilustra el papel pr\'actico de la fuerza bruta en el proyecto: para instancias peque\~nas, sirve como mecanismo de validaci\'on exacta; para instancias grandes, su crecimiento factorial la vuelve inviable.

\subsubsection{An\'alisis de combinaciones y l\'imites computacionales}

Para profundizar en la aplicaci\'on del algoritmo y evaluar el comportamiento de la fuerza bruta en escenarios m\'as complejos planteados por el equipo ($n=10$ y $n=12$), se analiza cuantitativamente la cantidad de combinaciones (permutaciones) evaluadas en funci\'on del n\'umero de tablones. Debido a que el espacio de b\'usqueda crece de forma factorial ($n!$), un incremento lineal en el tama\~no de la finca desencadena una explosi\'on combinatoria.

La siguiente tabla resume el total de permutaciones peri\'odicas que la funci\'on \texttt{roFB} debe examinar exhaustivamente para determinar el m\'inimo global:
...
\begin{table}[H]
  ...
  \begin{tabular}{cc}
    \toprule
    Tama\~no de la finca (\(n\)) & Permutaciones totales evaluadas (\(n!\)) \\
    \midrule
    5  & 120 \\
    8  & 40,320 \\
    10 & 3,628,800 \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Escalabilidad del espacio de b\'usqueda y total de combinaciones evaluadas.}
\end{table}

Esta aplicaci\'on num\'erica evidencia por qu\'e el enfoque exhaustivo responde de inmediato con las instancias base ($120$ combinaciones), pero se vuelve impracticable al alcanzar los $10$ o $12$ tablones. Procesar casi 480 millones de combinaciones para $n=12$ satura r\'apidamente el tiempo de CPU en entornos convencionales, lo que justifica la implementaci\'on de estrategias m\'as eficientes como el enfoque voraz o la programaci\'on din\'amica.
...
\begin{lstlisting}[style=bloquecodigo,language=C++]
...
\begin{lstlisting}[style=pseudobloque,basicstyle=\ttfamily\normalsize\linespread{1.12}\selectfont]
...
La implementaci\'on del proyecto resuelve esta parte mediante programaci\'on din\'amica \emph{top-down} con memoizaci\'on sobre subconjuntos de tablones. La idea central es modelar cada subproblema por el conjunto de tablones que ya fueron regados y calcular, para ese estado, el costo m\'inimo restante. Esta formulaci\'on sigue los cuatro pasos cl\'asicos de la programaci\'on din\'amica.

\subsubsection{Paso 1: Caracterizar la estructura de la soluci\'on \'optima}

En lugar de pensar directamente en permutaciones completas, la estrategia observa el problema por \textbf{estados}. Un estado queda determinado por el conjunto de tablones que ya fueron regados. Si llamamos \(M\) a ese conjunto, entonces el tiempo acumulado hasta ese punto queda completamente determinado por los tablones de \(M\). Lo representamos por
\[
\tau(M)=\sum_{i \in M} tr_i.
\]

Aqu\'i es importante distinguir dos cantidades diferentes: \(tr_i\) sigue representando la \textbf{duraci\'on del riego} del tabl\'on \(i\), mientras que \(\tau(M)\) representa el \textbf{instante acumulado} en el que comenzar\'ia a regarse el siguiente tabl\'on despu\'es de completar los de \(M\).

Con esta definici\'on, el subproblema asociado a \(M\) puede expresarse as\'i:

\begin{quote}
determinar el costo m\'inimo adicional necesario para completar el riego de todos los tablones que a\'un no pertenecen a \(M\), comenzando en el instante \(\tau(M)\).
\end{quote}

La programaci\'on din\'amica es aplicable porque se cumplen las dos propiedades fundamentales:
\begin{itemize}
  \item \textbf{Subestructura \'optima:} si desde un estado \(M\) la mejor decisi\'on es regar primero el tabl\'on \(i\), entonces el resto del plan debe ser tambi\'en \'optimo para el nuevo estado \(M \cup \{i\}\).
  \item \textbf{Solapamiento de subproblemas:} distintos \'ordenes parciales pueden llevar al mismo conjunto \(M\), de modo que el mismo subproblema reaparece muchas veces si no se guarda su resultado.
\end{itemize}

\subsubsection{Paso 2: Definir recursivamente el valor de la soluci\'on \'optima}

Para cada tabl\'on \(i\), definimos su costo local al comenzar a regarse en el instante \(t\):
\[
\cost{i}(t)=
\begin{cases}
...
En esta expresi\'on, \(t\) denota el \textbf{instante de inicio del riego} del tabl\'on \(i\), mientras que \(tr_i\) denota el \textbf{tiempo que dura} su riego. Es decir, \(t\) indica cu\'ando empieza el tabl\'on y \(t+tr_i\) indica cu\'ando termina.

Ahora definimos \(DP(M)\) como el costo m\'inimo adicional a partir del estado \(M\). Si ya fueron regados todos los tablones, no queda ning\'un costo por agregar. En caso contrario, se prueba cada tabl\'on que a\'un no ha sido regado y se elige la mejor continuaci\'on. Eso produce la recurrencia:
\[
DP(M)=0 \qquad \text{si } M \text{ contiene todos los tablones,}
\]
\[
DP(M)=\min_{i \notin M}\left\{ \cost{i}\bigl(\tau(M)\bigr) + DP(M \cup \{i\}) \right\}
\qquad
\text{en otro caso.}
\]

En palabras: desde el estado \(M\), el algoritmo intenta regar como siguiente tabl\'on a cada \(i \notin M\); para cada opci\'on suma el costo inmediato de regar \(i\) en el instante actual y el mejor costo posible del subproblema que queda despu\'es.

\subsubsection{Paso 3: Calcular el valor de la soluci\'on \'optima}

La implementaci\'on concreta usa memoizaci\'on y esto se observa directamente en la funci\'on \texttt{Resolver(M)} del pseudoc\'odigo. Su funcionamiento puede leerse en cuatro acciones:
\begin{itemize}
  \item primero verifica si \(M\) ya contiene todos los tablones; si eso ocurre, retorna \(0\), que es el caso base;
  \item luego consulta si \texttt{dp[M]} ya est\'a definido; si lo est\'a, reutiliza ese valor y evita recomputar el subproblema;
  \item si el estado todav\'ia no ha sido resuelto, calcula el instante actual \(t \leftarrow \tau(M)\) y recorre todos los tablones que a\'un no pertenecen a \(M\);
  \item para cada candidato \(i\), eval\'ua el costo
  \[
  c_i(t) + \texttt{Resolver}(M \cup \{i\}),
  \]
  conserva el menor valor encontrado y finalmente lo almacena en \texttt{dp[M]}.
\end{itemize}

As\'i, el pseudoc\'odigo implementa exactamente la recurrencia del paso anterior y garantiza que cada estado se calcula una sola vez.

\subsubsection{Paso 4: Construir una soluci\'on \'optima}

La reconstrucci\'on tambi\'en puede leerse directamente en el pseudoc\'odigo. Cada vez que \texttt{Resolver(M)} encuentra un mejor candidato, no solo actualiza \texttt{dp[M]}, sino que guarda en \texttt{eleccion[M]} cu\'al fue el tabl\'on que produjo ese mejor costo. Despu\'es, la funci\'on \texttt{roPD} usa esa informaci\'on de la siguiente manera:
\begin{itemize}
  \item comienza en el estado vac\'io, es decir, cuando todav\'ia no se ha regado ning\'un tabl\'on;
  \item consulta \texttt{eleccion[M]} para saber qu\'e tabl\'on debe regarse a continuaci\'on en una soluci\'on \'optima;
  \item agrega ese tabl\'on al vector \texttt{orden} y actualiza el estado a \(M \cup \{i\}\);
  \item repite el proceso hasta que el estado contiene todos los tablones.
\end{itemize}

De esta manera, el pseudoc\'odigo no solo calcula el costo \'optimo, sino que adem\'as reconstruye expl\'icitamente una permutaci\'on que alcanza ese costo.

El siguiente pseudoc\'odigo resume los pasos 3 y 4 de la estrategia:

\begin{center}
\begin{minipage}{0.92\textwidth}
\begin{lstlisting}[style=pseudobloque]
Funcion Resolver(M)
    si M = todos los tablones
        retornar 0
...
Funcion roPD(tablones)
    ...
    mientras M no contenga todos los tablones
        i <- eleccion[M]
        agregar i a orden
        M <- M U {i}

    retornar evaluar_orden_completo(tablones, orden)
\end{lstlisting}
\end{minipage}
\end{center}
...
La estrategia descrita arriba se refleja directamente en la funci\'on \texttt{roPD} implementada en [backend/src/algoritmos.cpp]. El siguiente fragmento muestra su n\'ucleo:
...
\begin{itemize}
  \item \verb|limite_estados = 1 << n| materializa que la DP trabaja sobre los \(2^n\) subconjuntos posibles de tablones.
  \item \texttt{dp[mascara]} almacena el costo \'optimo del subproblema asociado al estado \(M\), es decir, al conjunto de tablones ya regados.
  \item \texttt{elecciones[mascara]} guarda qu\'e tabl\'on produce el mejor valor de la recurrencia y permite reconstruir una permutaci\'on \'optima al finalizar.
  \item La lambda \texttt{resolver\_dp(..., mascara, dia\_actual)} implementa el subproblema \(DP(M)\) usando tanto la m\'ascara del estado como el instante \(\tau(M)\) en el que comenzar\'ia el siguiente riego.
  \item La condici\'on \verb|mascara == limite_estados - 1| es el caso base: si todos los tablones ya fueron regados, el costo adicional es \(0\).
  \item La prueba \verb|dp[mascara] != -1| implementa la memoizaci\'on: si el estado ya fue resuelto, el algoritmo reutiliza ese valor y evita recomputarlo.
  \item La condici\'on \verb|(mascara & (1 << i)) == 0| selecciona los tablones que todav\'ia no pertenecen al conjunto \(M\).
  \item \texttt{calcular\_costo(tablones[i], dia\_actual)} implementa exactamente la funci\'on local \(\cost{i}(t)\) en el instante actual.
  \item La llamada recursiva enciende el bit del tabl\'on \(i\) en la m\'ascara actual y suma su tiempo de riego al instante acumulado; eso corresponde exactamente a la transici\'on \(M \to M \cup \{i\}\).
  \item La actualizaci\'on de \texttt{mejor\_costo} y \texttt{mejor\_siguiente\_tablon} realiza el m\'inimo de la recurrencia y conserva la decisi\'on necesaria para reconstruir la soluci\'on.
\end{itemize}

La fase de reconstrucci\'on aparece inmediatamente despu\'es en la misma funci\'on:
...
Este ciclo implementa de manera literal el paso 4 de la estrategia: empieza en el estado vac\'io, consulta la mejor elecci\'on almacenada para ese estado, agrega el tabl\'on correspondiente al orden y avanza al nuevo subconjunto hasta alcanzar el estado completo. Por eso la implementaci\'on no solo calcula el valor \'optimo, sino que adem\'as construye expl\'icitamente una permutaci\'on que alcanza ese costo.

\subsection{An\'alisis de Complejidad en Tiempo y en Espacio}

\paragraph{Proposici\'on.}
La complejidad en tiempo de la programaci\'on din\'amica descrita arriba es \(O(n2^n)\) y su complejidad en espacio es \(O(2^n)\).

\paragraph{Demostraci\'on.}
La clave del an\'alisis est\'a en contar cu\'antos estados distintos puede tomar la funci\'on \texttt{Resolver} y cu\'anto cuesta procesar cada uno.

Cada estado est\'a determinado por un subconjunto \(M \subseteq U\). Como \(U\) tiene \(n\) elementos, el n\'umero total de subconjuntos posibles es:
\[
|\mathcal{P}(U)|=2^n.
\]

Debido a la memoizaci\'on, cada estado \(M\) se resuelve a lo sumo una vez. Cuando vuelve a aparecer, el pseudoc\'odigo retorna inmediatamente el valor almacenado en \texttt{dp[M]}. Por tanto, el costo total queda determinado por el procesamiento de esos \(2^n\) estados distintos.

En cada estado, el bloque dominante es el ciclo
\[
\texttt{para cada } i \in U \setminus M.
\]
En el peor caso, este ciclo examina \(n\) tablones. Cada iteraci\'on realiza solo operaciones de tiempo constante: verificar pertenencia mediante la m\'ascara, evaluar \(\cost{i}(t)\), consultar o invocar recursivamente un estado memoizado y comparar costos. Luego, el costo por estado es \(O(n)\).

En consecuencia, el costo total de calcular todos los valores de la tabla din\'amica es:
\[
2^n \cdot O(n)=O(n2^n).
\]

La fase de reconstrucci\'on recorre exactamente una vez los \(n\) tablones del orden \'optimo, de modo que cuesta \(O(n)\). Esta fase no altera la cota dominante. Por tanto, la complejidad en tiempo total del algoritmo es:
\[
O(n2^n).
\]

En espacio, las estructuras dominantes son los arreglos \texttt{dp} y \texttt{eleccion}, ambos indexados por m\'ascara y de tama\~no \(2^n\). Adem\'as, la recursi\'on y el vector del orden reconstruido aportan a lo sumo \(O(n)\) posiciones adicionales. Desde el punto de vista asint\'otico, las constantes de esas tablas no alteran la cota, aunque en la estimaci\'on pr\'actica s\'i importa distinguir que en la implementaci\'on real \texttt{dp} guarda costos y \texttt{elecciones} guarda \'indices. Por ello,
\[
O(2^n) + O(n)=O(2^n),
\]
...
Bajo la hip\'otesis del enunciado de \(3\times 10^8\) operaciones por minuto y 4 bytes por celda, y contando las dos tablas dominantes del algoritmo (\texttt{dp} y \texttt{eleccion}), la memoria base abstracta es aproximadamente \(8 \cdot 2^n\) bytes. En la implementaci\'on real del proyecto, \texttt{dp} se almacena como \texttt{long long} y \texttt{elecciones} como \texttt{int}, de modo que la memoria base efectiva se aproxima mejor por \(12 \cdot 2^n\) bytes. La tabla siguiente no corresponde a una corrida experimental real con \(n=32\), sino a una \textbf{proyecci\'on anal\'itica} obtenida a partir de las cotas \(O(n2^n)\) y \(O(2^n)\) bajo las hip\'otesis del enunciado. Con esa referencia, se obtiene el siguiente panorama para \(n = 2^k\):

\begin{table}[H]
  \centering
  \footnotesize
  \begin{tabularx}{\textwidth}{>{\centering\arraybackslash}p{0.07\textwidth}>{\centering\arraybackslash}p{0.11\textwidth}>{\centering\arraybackslash}p{0.20\textwidth}>{\centering\arraybackslash}p{0.15\textwidth}>{\centering\arraybackslash}X>{\centering\arraybackslash}X}
    \toprule
    \(k\) & \(n=2^k\) & \(n2^n\) operaciones & Tiempo aprox. & \shortstack{Memoria abstracta\\\(8 \cdot 2^n\)} & \shortstack{Memoria real\\\(12 \cdot 2^n\)} \\
    \midrule
    ...
    \bottomrule
  \end{tabularx}
  \caption{Estimaci\'on pr\'actica de la programaci\'on din\'amica, distinguiendo la memoria abstracta del an\'alisis y la memoria base de la implementaci\'on real.}
\end{table}

\paragraph{Tiempo: valores aceptables e inaceptables en la pr\'actica.}
Bajo la hip\'otesis del enunciado, el tiempo que se tomar\'ia el equipo en resolver el problema puede interpretarse as\'i:
\begin{itemize}
  \item \textbf{\(k=3\) (\(n=8\)): aceptable.} El algoritmo requiere \(2{,}048\) operaciones, lo que equivale a un tiempo despreciable frente a la capacidad de \(3\times 10^8\) operaciones por minuto.
  \item \textbf{\(k=4\) (\(n=16\)): aceptable.} El costo sube a \(1{,}048{,}576\) operaciones, pero eso sigue representando apenas \(0.0035\) minutos, es decir, una fracci\'on muy peque\~na de un minuto.
  \item \textbf{\(k=5\) (\(n=32\)): inaceptable.} El costo alcanza \(137{,}438{,}953{,}472\) operaciones, que se traducen en aproximadamente \(458\) minutos. Un tiempo de varias horas deja de ser razonable para uso pr\'actico ordinario.
\end{itemize}

Por tanto, para los valores analizados, el tiempo es \textbf{aceptable} para \(k=3\) y \(k=4\), pero se vuelve \textbf{inaceptable} a partir de \(k=5\).

\paragraph{Espacio: valores aceptables e inaceptables en la pr\'actica.}
Bajo la misma hip\'otesis, el espacio que tomar\'ia el equipo en resolver el problema puede interpretarse as\'i:
\begin{itemize}
  \item \textbf{\(k=3\) (\(n=8\)): aceptable.} Tanto la memoria abstracta (\(2\) KB) como la memoria base de la implementaci\'on real (\(3\) KB) son completamente manejables.
  \item \textbf{\(k=4\) (\(n=16\)): aceptable.} La memoria abstracta es de \(512\) KB y la memoria base real es de \(768\) KB, valores todav\'ia peque\~nos en la pr\'actica.
  \item \textbf{\(k=5\) (\(n=32\)): inaceptable.} Incluso en la estimaci\'on abstracta ya se requieren \(32\) GB para las dos tablas principales, y en la implementaci\'on real esa base sube a \(48\) GB. Ese consumo deja de ser razonable para un entorno de trabajo ordinario.
\end{itemize}

Por tanto, para los valores analizados, el espacio es \textbf{aceptable} para \(k=3\) y \(k=4\), pero se vuelve \textbf{inaceptable} a partir de \(k=5\).

\paragraph{Conclusi\'on sobre utilidad pr\'actica.}
La programaci\'on din\'amica es perfectamente viable para tama\~nos peque\~nos y medianos, pero deja de ser pr\'actica cuando \(n\) supera los 20 tablones aproximadamente. A partir de \(n=32\), seg\'un esta proyecci\'on anal\'itica, tanto el tiempo como el espacio crecen demasiado r\'apido. Aun as\'i, constituye una mejora enorme frente a la fuerza bruta, cuyo crecimiento factorial la vuelve inviable mucho antes: mientras la fuerza bruta no escala m\'as all\'a de \(n \approx 10\), la PD alcanza c\'omodamente \(n = 20\) y en la pr\'actica del proyecto resolvi\'o todas las instancias de la bater\'ia con exactitud garantizada.

\subsection{Correcci\'on}

\paragraph{Teorema.}
Para toda finca \(F\) con conjunto de tablones \(U\), la funci\'on \texttt{roPD} devuelve una permutaci\'on \(\widehat{\Pi}\) tal que
\[
CR_F^{\widehat{\Pi}}=\min_{\Pi} CR_F^\Pi.
\]

\paragraph{Demostraci\'on.}
La prueba se hace por inducci\'on sobre el n\'umero de tablones que faltan por regar.

Para cada subconjunto \(M \subseteq U\), interpretamos \(DP(M)\) como el costo m\'inimo adicional desde el estado en el que ya fueron regados exactamente los tablones de \(M\) y el instante actual es \(\tau(M)\).

\textbf{Caso base.} Si \(M=U\), no queda ning\'un tabl\'on por regar. El costo adicional correcto es \(0\). El pseudoc\'odigo retorna precisamente \(0\), de modo que \(DP(U)\) es correcto.

\textbf{Paso inductivo.} Supongamos que para todo subconjunto \(M'\) con a lo sumo \(r\) tablones pendientes, la funci\'on \texttt{Resolver(M')} devuelve correctamente el costo \'optimo adicional desde ese estado. Consideremos ahora un subconjunto \(M\) con exactamente \(r+1\) tablones pendientes.

Cualquier programaci\'on factible que contin\'ue desde \(M\) debe escoger primero un tabl\'on \(i \notin M\). Si esa decisi\'on se toma en el instante \(\tau(M)\), el costo total de esa rama se descompone necesariamente como:
\[
\cost{i}\bigl(\tau(M)\bigr) + \text{(costo \'optimo del resto desde } M \cup \{i\}\text{)}.
\]
Por hip\'otesis inductiva, el costo \'optimo del resto es exactamente \(DP(M \cup \{i\})\). Por tanto, para cada \(i \notin M\), el mejor costo posible de una soluci\'on que empieza con \(i\) es:
\[
\cost{i}\bigl(\tau(M)\bigr) + DP(M \cup \{i\}).
\]

Como toda soluci\'on factible desde \(M\) debe empezar con alg\'un \(i \notin M\), el mejor costo posible desde \(M\) es precisamente el m\'inimo de esas cantidades:
\[
DP(M)=\min_{i \notin M}\left\{ \cost{i}\bigl(\tau(M)\bigr) + DP(M \cup \{i\}) \right\}.
\]
Eso es exactamente lo que calcula el pseudoc\'odigo. Luego \texttt{Resolver(M)} devuelve el costo \'optimo para todo subconjunto \(M\) con \(r+1\) tablones pendientes.

Por inducci\'on, la recurrencia es correcta para todo subconjunto \(M \subseteq U\), y en particular para \(M=\varnothing\). As\'i, \texttt{Resolver(vac\'io)} produce el costo \'optimo global.

Falta justificar la reconstrucci\'on. En cada estado \(M\), la variable \texttt{eleccion[M]} almacena un tabl\'on que alcanza el m\'inimo de la recurrencia. Al seguir iterativamente esas elecciones desde el estado vac\'io hasta \(U\), se construye una permutaci\'on \(\widehat{\Pi}\) cuyas decisiones locales coinciden con una cadena de subproblemas \'optimos. Por consiguiente, el costo de \(\widehat{\Pi}\) es exactamente \(DP(\varnothing)\), que ya se demostr\'o igual al m\'inimo global.

En conclusi\'on, \texttt{roPD} siempre devuelve una soluci\'on \'optima.
...
F_2=\langle\langle 9,3,4,0\rangle,\langle 5,3,3,2\rangle,\langle 2,2,1,0\rangle,\langle 8,1,1,6\rangle,\langle 6,4,2,1\rangle\rangle
\]
la programaci\'on din\'amica produce igualmente
\[
\Pi^\star=\langle 2,1,0,3,4\rangle
\qquad\text{con costo}\qquad
32.
\]
Frente a los 55 del voraz, el patr\'on de mejora es el mismo: atender primero \(T_2\), que tiene margen nulo, evita que entre al caso 3.

\subsubsection{Evaluaci\'on experimental}

En los 18 casos de la bater\'ia oficial del repositorio que cuentan con salida de referencia, la programaci\'on din\'amica coincidi\'o exactamente con el costo \'optimo en todos los casos. Los valores detallados por test se presentan m\'as adelante en las tablas comparativas de la Secci\'on 4; por esa raz\'on, en particular en las Tablas~\ref{tab:comparacion-costos}, \ref{tab:comparacion-costos-supervivencia} y \ref{tab:comparacion-costos-prioridad}, la columna \texttt{Costo PD} se usa como referencia \'optima para cuantificar las desviaciones del algoritmo voraz.

Para instancias m\'as grandes, los tiempos de ejecuci\'on crecieron de acuerdo con la complejidad exponencial del algoritmo. En particular, los casos mixtos con \(n=20\) siguieron siendo resolubles en tiempos pr\'acticos, mientras que a partir de \(n=32\) la estrategia deja de ser viable por el crecimiento combinado del tiempo y la memoria.
...
La programaci\'on din\'amica representa la alternativa adecuada cuando la informaci\'on es cr\'itica y obtener una soluci\'on \'optima justifica un mayor consumo de tiempo y memoria. La formalizaci\'on del problema mediante subestructura \'optima y recurrencia permite construir una soluci\'on exacta con fundamentos te\'oricos s\'olidos, y en el dise\~no de pruebas mostr\'o un desempe\~no correcto en los tama\~nos moderados considerados. No obstante, su complejidad en tiempo \(O(n2^n)\) y su complejidad en espacio \(O(2^n)\) obligan a delimitar cuidadosamente el tama\~no del problema: en este proyecto se mantuvo como opci\'on viable hasta alrededor de \(n=20\), pero a mayor escala tambi\'en termina volvi\'endose computacionalmente inviable.
...
\section*{Notas de implementaci\'on}
\addcontentsline{toc}{section}{Notas de implementaci\'on}

Esta nota resume de forma breve c\'omo est\'a organizada la implementaci\'on del proyecto y c\'omo se conectan sus componentes principales.

\begin{itemize}
  \item \textbf{N\'ucleo algor\'itmico.} La implementaci\'on de las tres estrategias solicitadas se encuentra en \texttt{backend/src/algoritmos.cpp}. All\'i se definen las funciones \texttt{roFB}, \texttt{roV} y \texttt{roPD}, cada una encargada de recibir una finca y devolver una soluci\'on compuesta por la permutaci\'on de riego y su costo total.
  \item \textbf{Tipos de dominio.} Las estructuras b\'asicas del problema, como \texttt{Tablon}, \texttt{ResultadoCalculo} y los datos auxiliares de salida, se declaran en \texttt{backend/include/domain\_types.hpp}. Esto permite que el c\'alculo algor\'itmico y la capa de interfaz compartan un mismo modelo de datos.
  \item \textbf{Lectura de entradas.} La transformaci\'on del texto de entrada hacia una finca interna se realiza en \texttt{backend/src/placeholder\_engine.cpp}. La primera l\'inea del archivo indica \(n\), y cada una de las siguientes \(n\) l\'ineas usa el formato \texttt{ts,tr,p,rp}.
  \item \textbf{Capa de servicio.} El backend expone rutas HTTP desde \texttt{backend/src/main.cpp}. En particular, la ruta \texttt{/calcular} recibe el algoritmo solicitado y el texto de la finca, ejecuta \texttt{roFB}, \texttt{roV} o \texttt{roPD}, y responde en formato JSON con costo, permutaci\'on y detalles por tabl\'on.
  \item \textbf{Interfaz.} La interfaz principal est\'a en \texttt{riego\_interfaz\_frontend.html} y en los m\'odulos de \texttt{assets/js}. Desde all\'i se puede cargar una finca, visualizar sus tablones, escoger el algoritmo, revisar el resultado calculado y exportar la salida.
  \item \textbf{Formato de salida.} Para efectos de exportaci\'on, la salida mantiene la estructura pedida en el enunciado: primera l\'inea con el costo total y luego \(n\) l\'ineas con los \'indices de los tablones en el orden en que deben ser regados.
  \item \textbf{Pruebas y validaci\'on.} Los casos de prueba del proyecto se encuentran en \texttt{bateria\_pruebas/}. Las entradas y salidas oficiales del repositorio se usaron para validar la exactitud de fuerza bruta y programaci\'on din\'amica, y para medir la desviaci\'on del algoritmo voraz frente al costo \'optimo.
\end{itemize}
